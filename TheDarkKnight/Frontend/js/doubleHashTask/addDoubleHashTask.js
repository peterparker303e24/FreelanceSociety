...
import { DOUBLE_HASH_TASK_CONTRACT_ADDRESS, THE_LIST_CONTRACT_ADDRESS, USERS_CONTRACT_ADDRESS } from "../../utils/constants.js";
...
const secondResponseWindow
    = document.getElementById("second-response-window-input");
const delay = document.getElementById("delay-input");
const rewardInput = document.getElementById("reward-input");
const checkbox = document.getElementById("checkbox");
const addDoubleHashTaskButton = document.getElementById("add-task-button");
...
// Validates second response window numerical input and update variables
secondResponseWindow.addEventListener("input", () => {
    updateInputNumberToGroupedDigits(secondResponseWindow);
    updateAddHashButton();
});

// Validates second response delay numerical input and update variables
delay.addEventListener("input", () => {
    updateInputNumberToGroupedDigits(delayValue);
    updateAddHashButton();
});
...
addDoubleHashTaskButton.addEventListener("click", async () => {
    ...
    try {
        transactionResponse = await doubleHashTaskSigner.addDoubleHashTask(
            prefixHexBytes(hashValue),
            prefixHexBytes(taskHash),
            secondsToDeadline,
            isKeyRevealOn,
            secondResponseWindowValue,
            delayValue,
            { value: reward }
        );
    } catch (error) {
...
function updateAddHashButton() {
    ...
    const validSecondResponseWindow = secondResponseWindowValue !== undefined
        && Number(secondResponseWindowValue) !== NaN
        && Number(secondResponseWindowValue) > 0;
    ...
    } else if (!validSecondResponseWindow) {
        errorText.textContent
            = `Invalid second response window, must be positive number`;
    } else if (!validDelay) {
        errorText.textContent = `Invalid delay, must be non-negative number`;
    } else if (!validReward) {
